 ...
 */
/*__global__
void init(Body *p, float *buf, int n){
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
  for(int i = index; i < n; i += stride)
  {
    Body body = ((Body*)buf)[i];
    p[i].x = body.x;
    p[i].y = body.y;
    p[i].z = body.z;
    p[i].vx=body.vx;
...
int main(const int argc, const char** argv) {
  ...
  //init<<<numberOfBlocks,threadsPerBlock>>>(p,buf,nBodies);
   ...
   */
    ...
    cudaMemcpyAsync(buf, p, bytes, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
